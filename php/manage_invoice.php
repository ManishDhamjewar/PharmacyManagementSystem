<?php
  ...
  function printInvoice($invoice_number) {
    ...
    <div class="row">
      ...
      $query = "SELECT * FROM admin_credentials ORDER BY UID DESC";
      $result = mysqli_query($con, $query);
      $row = mysqli_fetch_array($result);
        ...
        <table class="table table-bordered table-striped table-hover" id="purchase_report_div">
          ...
          <tbody>
            <?php
              $seq_no = 0;
              $total = 0;
              $query = "SELECT * FROM medicines_stock INNER JOIN invoices ON invoices.INVOICE_ID = medicines_stock.ID WHERE INVOICE_ID = $invoice_number";
              $result = mysqli_query($con, $query);
              while($row = mysqli_fetch_array($result)) {
                $seq_no++;
                ?>
